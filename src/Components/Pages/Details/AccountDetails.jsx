...
const AccountDetails = () => {
    useTitle('AccountDetails');
    const { accountType } = useParams();
    const [details, setDetails] = useState();

    useEffect(() => {
        fetch(`https://bank-server-salmatonka.vercel.app/accounts/${accountType}`)
            .then(res => res.json())
    ...
    return (
        ...
        <div>
            <div className='bg-gray-900'>
                ...
                <div className="grid gap-10  lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 lg:px-20  pt-14 pb-6  group">

                    {
                        details?.facilities?.map(facility => <div key={facility}>
                            <div className='text-gray-100 rounded-lg shadow-lg shadow-sky-700 h-full '>
                                <div className='py-10  items-center'>
                                    <h2 className='px-6 pb-8 text-3xl font-semibold group-hover:text-center group-hover:w-full'>{facility?.title}</h2>
                                    <p className='hover:bg-gray-200 group-hover:text-gray-800 p-6 group-hover:mb-0 '> {facility?.details}</p>
                                </div>
                            </div>
